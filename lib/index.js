...
exports.default = function (_ref) {
  ...
  var checkIsIterable = expression('input && (typeof input[Symbol.iterator] === \'function\' || Array.isArray(input))');
  ...
  function skipEnvironment(comments, opts) {
    ...
    var envs = pragmaEnvironments(comments);
    return !opts.only.some(function (env) {
      return envs[env];
    });
  }

  function pragmaEnvironments(comments) {
    var pragma = /@typecheck:\s*(.+)/;
    var environments = {};
    comments.forEach(function (comment) {
      var m = comment.value.match(pragma);
      if (m) {
        m[1].split(',').forEach(function (env) {
          return environments[env.trim()] = true;
        });
      }
    });
    return environments;
  ...
  var visitors = {
    ...
    ImportDeclaration: function ImportDeclaration(path) {
      ...
      var _path$get$map$reduce = path.get('specifiers').map(function (specifier) {
        ...
        return [declarators, specifiers];
          ...
          _path$get$map$reduce2 = _slicedToArray(_path$get$map$reduce, 2),
          declarators = _path$get$map$reduce2[0],
          specifiers = _path$get$map$reduce2[1];
        ...
        for (var _iterator = params[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          if (param.isObjectPattern() && param.node.typeAnnotation) {
            var _path$get = path.get('body'),
                _scope = _path$get.scope;

            var _id = _scope.generateUidIdentifier('arg' + param.key);
            var pattern = param.node;
            param.replaceWith(_id);
            if (path.node.expression) {
              var block = t.blockStatement([t.variableDeclaration('var', [t.variableDeclarator(pattern, _id)]), t.returnStatement(path.get('body').node)]);
              path.node.body = block;
              path.node.expression = false;
            } else {
              path.get('body.body')[0].insertBefore(t.variableDeclaration('var', [t.variableDeclarator(pattern, _id)]));
            }
          }
        }
      } catch (err) {
    ...
    Function: {
      enter: function enter(path, context) {
        var _node$body$body;
        ...
        var paramChecks = collectParamChecks(path, context);
      ...
      exit: function exit(path) {
        ...
        if (!node.returnCount && isVoid === false) {
          ...
          throw path.buildCodeFrameError(buildErrorMessage('Function ' + (node.id ? '"' + node.id.name + '" ' : '') + 'did not return a value.', annotation));
        }
    ...
    YieldExpression: function YieldExpression(path, context) {
      ...
      if (ok === true && !nextType) {
        ...
        throw path.buildCodeFrameError(buildErrorMessage('Function ' + (fn.node.id ? '"' + fn.node.id.name + '" ' : '') + 'yielded an invalid type.', yieldType, getAnnotation(path.get('argument'))));
      }
    ...
    ReturnStatement: function ReturnStatement(path, context) {
      var fn = path.getFunctionParent();
      if (!fn) {
        return;
      }
      ...
      var _fn$node = fn.node,
          returnType = _fn$node.returnType,
          returnGuardName = _fn$node.returnGuardName;
      ...
      if (!node.argument) {
        if (maybeNullableAnnotation(returnType) === false) {
          throw path.buildCodeFrameError(buildErrorMessage('Function ' + (fn.node.id ? '"' + fn.node.id.name + '" ' : '') + 'did not return a value.', returnType));
        }
        return;
      }
      ...
      if (ok === true) {
        ...
        throw path.buildCodeFrameError(buildErrorMessage('Function ' + (fn.node.id ? '"' + fn.node.id.name + '" ' : '') + 'returned an invalid type.', annotation, getAnnotation(path.get('argument'))));
      }
    ...
    VariableDeclaration: function VariableDeclaration(path, context) {
      if (maybeSkip(path)) {
        return;
      }
      var node = path.node,
      ...
      for (var i = 0; i < node.declarations.length; i++) {
        ...
        var _id2 = declaration.id,
            init = declaration.init;

        if (!_id2.typeAnnotation || _id2.hasBeenTypeChecked) {
          continue;
        }
        _id2.savedTypeAnnotation = _id2.typeAnnotation;
        _id2.hasBeenTypeChecked = true;
        var ok = staticCheckAnnotation(declarations[i], _id2.typeAnnotation);
        if (ok === true) {
          continue;
        } else if (ok === false) {
          throw path.buildCodeFrameError(buildErrorMessage('Invalid assignment value for "' + _id2.name + '".', _id2.typeAnnotation, getAnnotation(declarations[i])));
        }
        var check = checkAnnotation(_id2, _id2.typeAnnotation, scope);
        if (check) {
          collected.push(guard({
            check: check,
            message: varTypeErrorMessage(_id2, context)
          }));
        }
      }
      if (collected.length > 0) {
        var _check = collected.reduce(function (check, branch) {
          branch.alternate = check;
          return branch;
        });
        if (path.parent.type === 'Program' || path.parent.type === 'BlockStatement') {
          path.insertAfter(_check);
          ...
          if (children.length === 0) {
            body.replaceWith(_check);
          } else {
            children[0].insertBefore(_check);
          }
          ...
          path.parentPath.insertAfter(_check);
        } else {
          path.replaceWith(t.blockStatement([node, _check]));
        }
      }
    },
    AssignmentExpression: function AssignmentExpression(path, context) {
      if (maybeSkip(path)) {
        return;
      }
      var node = path.node,
      ...
      if (ok === true) {
        ...
        throw path.buildCodeFrameError(buildErrorMessage('Invalid assignment value for "' + humanReadableType(id) + '".', annotation, getAnnotation(right)));
      }
    ...
    ForOfStatement: function ForOfStatement(path, context) {
      if (maybeSkip(path)) {
        return;
      }
      var left = path.get('left');
      ...
      path.insertBefore(guard({
        ...
        message: t.binaryExpression('+', t.stringLiteral('Expected ' + humanReadableType(right.node) + ' to be iterable, got '), readableName({ inspect: context.inspect, input: id }))
      }));
      ...
      if (compareAnnotations(annotation, leftAnnotation) === false) {
        throw path.buildCodeFrameError(buildErrorMessage('Invalid iterator type.', annotation, leftAnnotation));
      }
    },
    ClassDeclaration: function ClassDeclaration(path, context) {
      ...
      try {
        for (var _iterator2 = path.get('body.body')[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          var memberPath = _step2.value;

          var classMember = memberPath.node;
      ...
      if (t.isGenericTypeAnnotation(type)) {
        var binding = path.scope.getBinding(type.id.name);
      ...
      var propTypes = t.objectExpression(type.properties.map(function (prop) {
        return t.objectProperty(t.identifier(prop.key.name), generatePropType(prop.value, prop.optional, path.scope, context));
      }));

      if (path.node.decorators) {
        var property = t.classProperty(t.identifier('propTypes'), propTypes);
        property.static = true;
        props.insertAfter(property);
      } else {
        var root = path.parentPath.isExportDeclaration() ? path.parentPath : path;
        root.insertAfter(t.expressionStatement(t.assignmentExpression("=", t.memberExpression(path.node.id, t.identifier("propTypes")), propTypes)));
      }
    }
  };
  ...
  function collectTypes(path) {
    path.traverse({
      ...
      "Function|Class": function FunctionClass(path) {
        var node = path.node;
        if (node.typeParameters && node.typeParameters.params) {
          path.get('typeParameters').get('params').forEach(function (typeParam) {
    ...
    visitor: {
      Program: function Program(path, _ref6) {
        ...
        if (!checkFile) {
          ...
          try {
            for (var _iterator4 = path.get('body')[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
        ...
        collectTypes(path);
        var inspect = path.scope.generateUidIdentifier('inspect');
        var requiresHelpers = {
          inspect: false
        };
        var context = {
   ...
   */
  function createFunctionReturnGuard(path, context) {
    var node = path.node,
        scope = path.scope;
  ...
  function createFunctionYieldGuards(path, context) {
    var node = path.node,
        scope = path.scope;
    ...
    if (yieldType) {
      ...
      var check = checkAnnotation(_id3, yieldType, scope);
      if (check) {
        node.yieldGuard = guardFn({
          id: _id3,
          name: _name,
          check: check,
          message: yieldTypeErrorMessage(node, yieldType, _id3, context)
        });
    ...
    if (nextType) {
      ...
      var _id4 = scope.generateUidIdentifier('id');
      var _check2 = checkAnnotation(_id4, nextType, scope);
      if (_check2) {
        node.nextGuard = guardFn({
          id: _id4,
          name: _name2,
          check: _check2,
          message: yieldNextTypeErrorMessage(node, nextType, _id4, context)
        });
  ...
  function buildErrorMessage(message, expected, got) {
    if (got) {
  ...
  function unionComparer(a, b, comparator) {
    ...
    try {
      for (var _iterator5 = a.types[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
  ...
  function intersectionComparer(a, b, comparator) {
    ...
    try {
      for (var _iterator6 = a.types[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
        var _type3 = _step6.value;
  ...
  function compareObjectAnnotation(a, b) {
    ...
    try {
      for (var _iterator7 = a.properties[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
        var aprop = _step7.value;
        ...
        try {
          for (var _iterator8 = b.properties[Symbol.iterator](), _step8; !(_iteratorNormalCompletion8 = (_step8 = _iterator8.next()).done); _iteratorNormalCompletion8 = true) {
            var bprop = _step8.value;
  ...
  function checkTypeof(_ref10) {
    ...
    switch (annotation.type) {
      case 'GenericTypeAnnotation':
        var _id5 = annotation.id;

        var path = Object.assign({}, input, { type: _id5.type, node: _id5, scope: scope });
  ...
  function checkObject(_ref23) {
    var input = _ref23.input,
        properties = _ref23.properties,
        indexers = _ref23.indexers,
        scope = _ref23.scope;
    ...
    var propNames = [];
    var check = properties.length === 0 ? checkIsObject({ input: input }) : properties.reduce(function (expr, prop, index) {
  ...
  function checkObjectPattern(_ref24) {
    ...
    try {
      for (var _iterator9 = input.properties[Symbol.iterator](), _step9; !(_iteratorNormalCompletion9 = (_step9 = _iterator9.next()).done); _iteratorNormalCompletion9 = true) {
        ...
        var check = checkAnnotation(_id6, prop.value, scope);
        if (check) {
  ...
  function checkAnnotation(input, annotation, scope, optional) {
    optional = optional || false;

    if (optional) {
      return t.logicalExpression('||', checks.undefined({ input: input }), checkAnnotation(input, annotation, scope, false));
    }
    ...
    switch (annotation.type) {
      ...
      case 'GenericTypeAnnotation':
        if (annotation.id.name === 'Array') {
          ...
          return checks.double({ input: input });
    ...
    if (node.type === 'TypeAlias') {
      ...
      switch (path.type) {
        case 'Identifier':
          ...
          var _id7 = binding.identifier;
          if (binding.path.type === 'ObjectPattern') {
            return getObjectPatternAnnotation(binding.path, node.name);
          }
          if (_id7.savedTypeAnnotation) {
            return _id7.savedTypeAnnotation;
          } else if (_id7.returnType) {
            return _id7.returnType;
          } else if (_id7.typeAnnotation) {
            return _id7.typeAnnotation;
          } else if (isPolymorphicType(_id7, scope)) {
  ...
  function getObjectPatternAnnotation(path, name) {
    ...
    try {
      for (var _iterator10 = path.get('properties')[Symbol.iterator](), _step10; !(_iteratorNormalCompletion10 = (_step10 = _iterator10.next()).done); _iteratorNormalCompletion10 = true) {
    ...
    var objectAnnotation = stack.reduceRight(function (last, target) {
      ...
      switch (annotation.type) {
        case 'AnyTypeAnnotation':
          return annotation;
        case 'ObjectTypeAnnotation':
          var _id8 = target.get('property').node;
          var _iteratorNormalCompletion13 = true;
          var _didIteratorError13 = false;
          var _iteratorError13 = undefined;

          try {
            for (var _iterator13 = (annotation.properties || [])[Symbol.iterator](), _step13; !(_iteratorNormalCompletion13 = (_step13 = _iterator13.next()).done); _iteratorNormalCompletion13 = true) {
              var _step13$value = _step13.value,
                  key = _step13$value.key,
                  value = _step13$value.value;

              if (key.name === _id8.name) {
  ...
  function getComputedMemberExpressionAnnotation(path) {
    ...
    var _property$evaluate = property.evaluate(),
        confident = _property$evaluate.confident,
        value = _property$evaluate.value;
      ...
      case 'UnionTypeAnnotation':
        ...
        try {
          for (var _iterator20 = annotation.types[Symbol.iterator](), _step20; !(_iteratorNormalCompletion20 = (_step20 = _iterator20.next()).done); _iteratorNormalCompletion20 = true) {
            var _type11 = _step20.value;

            var _result = maybeInstanceOfAnnotation(_type11, expected, typeParameters);
            if (_result === true) {
              return true;
  ...
  function maybeArrayAnnotation(annotation) {
    switch (annotation.type) {
      ...
      case 'UnionTypeAnnotation':
        ...
        try {
          for (var _iterator21 = annotation.types[Symbol.iterator](), _step21; !(_iteratorNormalCompletion21 = (_step21 = _iterator21.next()).done); _iteratorNormalCompletion21 = true) {
            var _type12 = _step21.value;
  ...
  function maybeIterableAnnotation(annotation) {
    switch (annotation.type) {
      ...
      case 'UnionTypeAnnotation':
        ...
        try {
          for (var _iterator22 = annotation.types[Symbol.iterator](), _step22; !(_iteratorNormalCompletion22 = (_step22 = _iterator22.next()).done); _iteratorNormalCompletion22 = true) {
            var _type13 = _step22.value;
  ...
  function maybeTupleAnnotation(annotation) {
    switch (annotation.type) {
      ...
      case 'UnionTypeAnnotation':
        ...
        try {
          for (var _iterator23 = annotation.types[Symbol.iterator](), _step23; !(_iteratorNormalCompletion23 = (_step23 = _iterator23.next()).done); _iteratorNormalCompletion23 = true) {
            var _type14 = _step23.value;
  ...
  function humanReadableType(annotation) {
    switch (annotation.type) {
      ...
      case 'GenericTypeAnnotation':
        var path = getNodePath(annotation);
        var checker = path && getTypeChecker(annotation.id, path.scope);
  ...
  function getTypeChecker(id, scope) {
    var checker = scope.getData('typechecker:' + id.name);
    if (checker) {
      return checker;
    }
    return false;
  ...
  function getPolymorphicType(id, scope) {
    var path = scope.getData('typeparam:' + id.name);
    if (path) {
      return path.node;
    }
  }

  function collectParamChecks(path, context) {
  ...
  function createParamGuard(path, context) {
    var node = path.node,
        scope = path.scope;

    node.hasBeenTypeChecked = true;
    node.savedTypeAnnotation = node.typeAnnotation;
    var checkable = void 0;
    ...
    var check = checkAnnotation(checkable, node.typeAnnotation, scope);
    if (!check) {
      return;
    }
    if (node.optional) {
      check = t.logicalExpression('||', checks.undefined({ input: checkable }), check);
    }
    var message = paramTypeErrorMessage(checkable, context, node.typeAnnotation);
    return guard({
      check: check,
      message: message
    });
  ...
  function createDefaultParamGuard(path, context) {
    var node = path.node,
        scope = path.scope;
    var id = node.left,
        value = node.right;
    ...
    if (ok === false) {
      throw path.buildCodeFrameError(buildErrorMessage('Invalid default value for argument "' + id.name + '".', id.typeAnnotation, getAnnotation(path.get('right'))));
    }
    return createParamGuard(path.get('left'), context);
  }

  function createRestParamGuard(path, context) {
    var node = path.node,
        scope = path.scope;
    ...
    if (isStrictlyArrayAnnotation(node.typeAnnotation) === false) {
      throw path.buildCodeFrameError(buildErrorMessage('Invalid type annotation for rest argument "' + id.name + '".', t.genericTypeAnnotation(t.identifier('Array')), node.typeAnnotation));
    }
    ...
    var message = paramTypeErrorMessage(id, context, node.typeAnnotation);
    return guard({
      check: check,
      message: message
    });
  ...
  function returnTypeErrorMessage(path, fn, id, context) {
    var node = path.node,
        scope = path.scope;
    ...
    var message = 'Function ' + (name ? '"' + name + '" ' : '') + 'return value violates contract.\n\nExpected:\n' + humanReadableType(annotation) + '\n\nGot:\n';

    return t.binaryExpression('+', t.stringLiteral(message), id ? readableName({ inspect: context.inspect, input: id }) : node.argument ? readableName({ inspect: context.inspect, input: node.argument }) : t.stringLiteral('undefined'));
  }

  function yieldTypeErrorMessage(fn, annotation, id, context) {
    var name = fn.id ? fn.id.name : '';
    var message = 'Function ' + (name ? '"' + name + '" ' : '') + 'yielded an invalid value.\n\nExpected:\n' + humanReadableType(annotation) + '\n\nGot:\n';

    return t.binaryExpression('+', t.stringLiteral(message), readableName({ inspect: context.inspect, input: id }));
  }
  function yieldNextTypeErrorMessage(fn, annotation, id, context) {
    var name = fn.id ? fn.id.name : '';
    var message = 'Generator ' + (name ? '"' + name + '" ' : '') + 'received an invalid next value.\n\nExpected:\n' + humanReadableType(annotation) + '\n\nGot:\n';

    return t.binaryExpression('+', t.stringLiteral(message), readableName({ inspect: context.inspect, input: id }));
  }

  function paramTypeErrorMessage(node, context) {
    var typeAnnotation = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : node.typeAnnotation;

    var name = node.name;
    if (node.type === 'MemberExpression' && node.object.name === 'arguments') {
      name = node.property.value;
    }
    var message = 'Value of ' + (node.optional ? 'optional ' : '') + 'argument ' + JSON.stringify(name) + ' violates contract.\n\nExpected:\n' + humanReadableType(typeAnnotation) + '\n\nGot:\n';

    return t.binaryExpression('+', t.stringLiteral(message), readableName({ inspect: context.inspect, input: node }));
  }

  function varTypeErrorMessage(node, context) {
    var annotation = node.typeAnnotation;
    if (node.type === 'Identifier') {
      var _name3 = node.name;
      var message = 'Value of variable "' + _name3 + '" violates contract.\n\nExpected:\n' + humanReadableType(annotation) + '\n\nGot:\n';
      return t.binaryExpression('+', t.stringLiteral(message), readableName({ inspect: context.inspect, input: node }));
    } else {
      var _message = 'Value of "' + humanReadableType(node) + '" violates contract.\n\nExpected:\n' + humanReadableType(annotation) + '\n\nGot:\n';
   ...
   */
  function generatePropType(annotation, optional, scope, context) {
    var prop = t.identifier('prop');
    var check = checkAnnotation(prop, annotation, scope, optional);
    if (check) {
      return propType({
        check: check,
        prop: prop,
   ...
   */
  function mustCheckFile(path, opts) {
   ...
   */
  function maybeSkipFile(path, opts) {
    if (path.node.leadingComments && path.node.leadingComments.length) {
